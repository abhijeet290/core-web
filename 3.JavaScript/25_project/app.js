...
function getRestaurants(reataurants) {
  const root = document.getElementById("root");
  reataurants.forEach((restaurant) => {
    // create a card
    // 1: Image
    // 2: Card_content
...
document.getElementById("Alcohol").addEventListener("click", () => {
  const result = restaurants.filter((rest) => rest.alcohol == true);
  // document.getElementById('root').innerHTML='' //both do the same thing but replaceChildren is better
  document.getElementById('root').replaceChildren()
  getRestaurants(result);
});
document.getElementById("Rating").addEventListener("click", () => {
  const result = restaurants.filter((rest) => rest.rating > 4.5);
